...
class Article(BaseModel):
    ...
    def save(self) -> 'Article':
        with sqlite3.connect('DATABASE_NAME','database.db') as con:
            cur = con.cursor()
            cur.execute(
                "INSERT INTO articles (id,author,title,content) VALUES(?, ?, ?, ?)",(self.id, self.author, self.title, self.content)
            )
            cur.commit()
    ...
    @classmethod
    def create_table(cls,database_name='databse.db'):
